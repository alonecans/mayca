...
const adm = ["6283893169044@s.whatsapp.net"]   //ganti nomor lu! 
/************************************/
...
var req = args.join(' ')            
	    ...
	    case 'setthumbb':
				...
				delb = await client.downloadMediaMessage(boij)
				fs.writeFileSync(`me.jpg`, delb)
				client.sendMessage(from, 'Sukses Wakil Owner😉',MessageType.text, { quoted: fvideo, contextInfo: { forwardingScore: 508, isForwarded: true}})
				break
		case 'tovid':
...
─────────────────────`
         ...
         if (isBanned) return reply('Maaf kamu sudah terbenned!')
         reply(ind.wait()) 
         if (args.length < 1) return reply(`Contoh : ${prefix + command} Tie Me Down`) 
         cari = body.slice(8) 
         nuy = await fetchJson(`https://videfikri.com/api/ytplayv2/?query=${cari}`) 
         infomp3 = `*Title :* ${nuy.result.title}\n*CH :* ${nuy.result.channel}\n*Upload :* ${nuy.result.published_on}\n*Durasi :* ${nuy.result.duration}\n*Views :* ${nuy.result.views}\n*Size :* ${nuy.result.size}\n*Download :* ${nuy.result.url}\n*Description :* ${nuy.result.description}`
         buffer = await getBuffer(nuy.result.thumbnail) 
         client.sendMessage(from, buffer, image, {quoted: fvideo, caption: infomp3})
         lagu = await getBuffer(nuy.result.url) 
         client.sendMessage(from, lagu, document, {mimetype: 'audio/mp3', filename: `${nuy.result.title}.mp3`, quoted: flokasi})
         await limitAdd(sender)
         break
		 case 'limit':
                 if (!isRegistered) return reply( ind.noregis())
