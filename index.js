	...
	if (!welkom.includes(anu.jid)) return
		try {
			...
			if (anu.action == 'add') {
				...
				buffer = await getBuffer('https://github.com/setyawan12/filemusikardan/blob/master/nurul(manca)/halo.mp3?raw=true')
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buffer, audio, {quoted: {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: 'Welcome Member', orderTitle: 'Welcome Member', thumbnail: fs.readFileSync('me.jpg'), sellerJid: '0@s.whatsapp.net'} } }, contextInfo: {"mentionedJid": [num]} mimetype: 'audio/mp4', ptt: true})
