	...
	fs.writeFile(`./${name}.exif`, buffer, (err) => {	
      ...
      if (budy.startsWith('>')){
		...
		case 'pinky':
           ...
           break
        case 'troli':
        if (!mek.key.froMe) return client.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./me.jpg`, 'base64')})
        reply("trolii!!!!!!")
        break
		case 'sewabot1':
...
var req = args.join(' ')            
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
        ...
        case 'kontag':
                    ...
                    client.sendMessage(from, {displayname: adan, vcard: vcard4}, MessageType.contact, {quoted: mek, contextInfo: {mentionedJid: mem}})
                    break
				case 'tempo':
				client.sendMessage(from, 'Sedang Diproses...',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    var req = args.join(" ")            
	...
	case 'emottoimg':
		...
		case 'shota':
               ...
               client.sendMessage(from, 'Sedang Diproses...',MessageType.text, { quoted: fgif, contextInfo: { forwardingScore: 508, isForwarded: true}})
				...
				client.sendMessage(from, buffer, video, {quoted: ftroli})
				await limitAdd(sender)
				break
					case 'meme': 
				if (!isRegistered) return client.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n${fxsx} ${pushname}\n${fxsx} Daftar Dulu Kak Dengan Cara\n${fxsx} ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./me.jpg`, 'base64')})
...
╰──────────`
		...
		case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buffer, image, {quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./me.jpg`, 'base64'), caption: `*「 BC GROUP 」*\n\nDari Grup : ${groupName}\nPengirim : wa.me/${(sender.split('@')[0])}\nPesan : ${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
...
buffer = await getBuffer('https://raw.githubusercontent.com/alonecans/foto-manca/main/me.jpg')
		...
		case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./me.jpg`, 'base64'), caption: `*「 PESAN BROADCAST 」*\n\n${body.slice(4)}`})
						}
						...
						for (let _ of anu) {
							sendMess(_.jid, {quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./me.jpg`, 'base64'), caption: `*「 BC BY OWNER 」*\n\n${body.slice(4)}`})
						}
						client.sendMessage(from, '*Suksess broadcast*',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./me.jpg`, 'base64')})
					}
					break
