	...
	if (!welkom.includes(anu.jid)) return
		try {
			...
			if (anu.action == 'add') {
				...
				let buffer = await getBuffer('https://github.com/setyawan12/filemusikardan/blob/master/nurul(manca)/halo.mp3?raw=true')
				let buff = await getBuffer(ppimg)
				client.sendMessage(from, buffer, audio, { mimetype: 'audio/mp4', ptt:true, quoted: {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: 'Welcome Member', orderTitle: 'Welcome Member', thumbnail: fs.readFileSync('me.jpg'), sellerJid: '0@s.whatsapp.net'} } }, contextInfo: {"mentionedJid": [num]}})
...
╰──────────`
	...
	client.on('message-new', async (mek) => {
		...
		if (urutan_pesan === 5000) {
			...
			const ftroli = {key : {participant : '6283815956151@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: fakereply, orderTitle: fakereply1, thumbnail: fs.readFileSync('me.jpg'), sellerJid: '6283815956151@s.whatsapp.net'} } }
			const isUrl = (url) => {
