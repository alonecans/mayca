...
function addMetadata(packname, author) {	
	...
	fs.writeFile(`./${name}.exif`, buffer, (err) => {	
      ...
      // Tebak Gambar
		...
		if (messagesC.includes("#izinadmin")) return client.sendMessage(from, '#izinadmin diterima',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})e.composing= `${sender.split("@")[0]}@s.whatsapp.net`
...
break
		case 'sewabot1':
                ...
                if (!isRegistered) return client.sendMessage(from, `*「 BELUM VERIFIKASI 」 *\n${fxsx} ${pushname}\n${fxsx} Daftar Dulu Kak Dengan Cara\n${fxsx} ${prefix}daftar pinky|16`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
...
─────────────────────`
       ...
       if (!isRegistered) return client.sendMessage(from, `*「 BELUM VERIFIKASI 」 *\n${fxsx} ${pushname}\n${fxsx} Daftar Dulu Kak Dengan Cara\n${fxsx} ${prefix}daftar pinky|16`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
        ...
        case 'apakah':
               ...
               if (!isRegistered) return client.sendMessage(from, `*「 BELUM VERIFIKASI 」 *\n${fxsx} ${pushname}\n${fxsx} Daftar Dulu Kak Dengan Cara\n${fxsx} ${prefix}daftar pinky|16`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
               if (isBanned) return reply('Maaf kamu sudah terbenned!')
               data = fs.readFileSync('./lib/pinky.js')
               jsonData = JSON.parse(data)
               randIndex = Math.floor(Math.random() * jsonData.length);
               randKey = jsonData[randIndex];
               pinky = await getBuffer(randKey.result)
               client.sendMessage(from, pinky, MessageType.audio, { mimetype: 'audio/mp4', ptt:true, quoted: fgif })
               await limitAdd(sender)
               break
		case 'bagaimanakah':
