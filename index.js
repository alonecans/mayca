...
─────────────────────`
                    ...
                    await client.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_result.info.song}.mp3`, quoted: mek2, duration: 99999999999, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    break
          case 'play2':
		         if (!isRegistered) return reply( ind.noregis())
				 if (isBanned) return reply('Maaf kamu sudah terbenned!')
		...
		case 'img2':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					client.sendMessage(from, pok, image, { quoted: {key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./me.jpg`)},"productId":"3937202479680283","title":"MANCA BOT","currencyCode":"IDR","priceAmount1000":"999999999000","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}, caption: `*Img ${command*`})
					await limitAdd(sender)
					break 
		   case 'pinterest':
                    if (!isRegistered) return reply( ind.noregis())
                    ...
                    client.sendMessage(from, buffer, image, { quoted: mek2, caption: `Image : ${command}` })
                    break
           case 'img':
...
─────────────────────`
       ...
       if (!isRegistered) return reply(ind.noregis())
        ...
        case 'apakah':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				  client.updatePresence(from, Presence.composing)
					const loli = fs.readFileSync('./pinky/ferguso.mp3')
					const loli = fs.readFileSync('./pinky/iya.mp3')
                    const loli = fs.readFileSync('./pinky/tidak.mp3')
                    const loli = fs.readFileSync('./pinky/mungkin iya.mp3')
                    const loli = fs.readFileSync('./pinky/mungkin tidak.mp3')
					const lolii = loli[Math.floor(Math.random() * loli.length)]
					client.sendMessage(from, lolii, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true, duration:999999999999})
					break
		case 'bagaimanakah':
