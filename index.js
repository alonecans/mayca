	...
	if (!welkom.includes(anu.jid)) return
		try {
			...
			if (anu.action == 'add') {
				...
				const loli = fs.readFileSync('./mp3/member.mp3')
				client.sendMessage(mdata.id, loli, MessageType.audio, {quoted: flokasi, contextInfo: {"mentionedJid": [num]}, mimetype: 'audio/mp4', ptt:true, duration:999999999999})
				client.sendMessage(mdata.id, buff, MessageType.image, {quoted: ftroli2, contextInfo: {"mentionedJid": [num]}, caption: teks})
...
╰──────────`
				...
				client.sendMessage(from, strConversation, MessageType.text, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true}})
...
╰──────────`
	...
	client.on('message-new', async (mek) => {
		...
		if (urutan_pesan === 5000) {
			...
			const ftroli = {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: fakereply, orderTitle: fakereply1, thumbnail: fs.readFileSync('me.jpg'), '0@s.whatsapp.net'} } }
			const ftroli2 = {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: `Welcome @${num.split('@')[0]}`, orderTitle: `Welcome @${num.split('@')[0]}`, thumbnail: try {ppimg = await client.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)}, catch {ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'}, let buff = await getBuffer(ppimg), '0@s.whatsapp.net'} } }
			const isUrl = (url) => {
...
─────────────────────`
       ...
       if (!isRegistered) return client.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n${fxsx} @${pushname}\n${fxsx} Daftar Dulu Kak Dengan Cara\n${fxsx} ${prefix}daftar pinky|16`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./me.jpg`, 'base64')})
        ...
        case 'apakahh':
               ...
               client.sendMessage(from, buffer, audio, { mimetype: 'audio/mp4', ptt:true, quoted: fgif })
               await limitAdd(sender)
               break
		case 'bagaimanakah':
...
${fxsx} NuyFaa
╰──────────` 
buffer = await getBuffer('https://raw.githubusercontent.com/alonecans/terserah/main/bynuy.jpg')
                    client.sendMessage(from, buffer, image, {quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [num]}, displayname: mem, caption: menusu })
					break
                  case 'addstic':
                case 'adds':
				    if (!isRegistered) return client.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n${fxsx} @${pushname}\n${fxsx} Daftar Dulu Kak Dengan Cara\n${fxsx} ${prefix}daftar pinky|16`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./me.jpg`, 'base64')})
